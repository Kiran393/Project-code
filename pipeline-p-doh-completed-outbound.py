...
spark.sql("select * from ( select tr.personid AS btpersonid ,tr.duedate AS btduedate ,tr.trainingprogram AS bttraining ,tr.completeddate AS btcompleted ,tr.earnedhours AS btearnedhours ,tr.trackingdate AS bttrackingdate ,tr.trainingprogramcode as bttrainingprogramcode ,ROW_NUMBER() OVER(PARTITION BY tr.personid ORDER BY tr.trainingid desc ,tr.completeddate desc) as trRank from  trainingrequirement tr WHERE tr.trackingdate >= '2013-09-01 00:00:00' and tr.trainingprogramcode = '201' and tr.completeddate  is not null and tr.completeddate <= tr.duedate and tr.earnedhours = 70 ) tr where tr.trRank = 1").createOrReplaceTempView("bt70trainingcompletedcte_tr")
...
outboundDF = spark.sql("SELECT distinct per.personid, per.lastname as lastname, per.firstname as firstname, per.ssn, per.dob as birthdate, upper(per.mailingstreet1) as mailingstreet1 , upper(per.mailingcity) as mailingcity, upper(per.mailingstate) as mailingstate, per.mailingzip, per.county, per.mobilephone, serv.activeemployers, date_format(bt.bttrackingdate, 'MM/dd/yyyy') as bttrackingdate, date_format(bt.btduedate, 'MM/dd/yyyy') as btduedate, cast(os.osearnedhours as int) as osearnedhours, date_format(os.oscompleted, 'MM/dd/yyyy') as oscompleted , cast(bt.btearnedhours as int) as btearnedhours, date_format(bt.btcompleted, 'MM/dd/yyyy') as btcompleted, '0001' as trainingprogramcode, 'Training Partnership' as trainingprogramname, bt.btinstructorid, bt.btinstructorname, cred.credentialnumber FROM person per JOIN employmentrelationship serv ON per.personid = serv.personid JOIN btlearnertrainingcte bt ON bt.btpersonid = per.personid JOIN oslearnertrainingcte os ON os.ospersonid = per.personid JOIN personhistory prc ON per.personid = prc.personid JOIN credentialcte cred ON cred.credentialnumber = prc.credentialnumber WHERE upper(per.status) = 'ACTIVE'")
...
pandas_df.to_csv("s3://seiubg-b2bds-prod-feeds-fp7mk/Outbound/doh/completed/Completed_Prometric_BT75_Students_"+suffix+"_1.txt", header=True, index=None, sep='|')
